...
public class PermitAppService : ApplicationService, IPermitAppService
{
    private readonly IRepository<Permit, Guid> _repository;
    private readonly PermitManager _manager;

    public PermitAppService(
        IRepository<Permit, Guid> repository,
    ...
    // 获取单个详情
    public async Task<PermitDto> GetAsync(Guid id)
    {
        ...
        // 如果找不到抛出异常
        if (entity == null) throw new EntityNotFoundException(typeof(Permit), id);

        return ObjectMapper.Map<Permit, PermitDto>(entity);
    }

    // 获取分页列表
    public async Task<PagedResultDto<PermitDto>> GetListAsync(PagedAndSortedResultRequestDto input)
    {
        // 1. 获取 Queryable
        var queryable = await _repository.GetQueryableAsync();
        ...
        // 5. 映射并返回
        return new PagedResultDto<PermitDto>(
            totalCount,
            ObjectMapper.Map<List<Permit>, List<PermitDto>>(entitys)
        );
    }

    // 创建
    //[Authorize(PCMPermissions.Projects.Create)]
    public async Task<PermitDto> CreateAsync(CreateUpdatePermitDto input)
    {
        ...
        var dto = ObjectMapper.Map<Permit, PermitDto>(entity);
    ...
    public async Task<PermitDto> UpdateAsync(Guid id, CreateUpdatePermitDto input)
    {
        ...
        if (entity == null)
        {
            throw new EntityNotFoundException(typeof(Permit), id);
        }
