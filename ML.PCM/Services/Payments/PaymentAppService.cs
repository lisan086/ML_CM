...
using ML.PCM.Managers.Payments;
...
public class PaymentAppService : ApplicationService, IPaymentAppService
{
    ...
    public async Task<PaymentDto> GetAsync(Guid id)
    {
        ...
        var entity = await AsyncExecuter.FirstOrDefaultAsync(
            queryable.Include(x => x.Contract).Where(x => x.Id == id)
        );

        // 如果找不到抛出异常
        if (entity == null) throw new EntityNotFoundException(typeof(Payment), id);
    ...
    public async Task<PagedResultDto<PaymentDto>> GetListAsync(PagedAndSortedResultRequestDto input)
    {
        ...
        return new PagedResultDto<PaymentDto>(
            totalCount,
            ObjectMapper.Map<List<Payment>, List<PaymentDto>>(entitys)
        );
    }
    ...
    public async Task<PaymentDto> CreateAsync(CreateUpdatePaymentDto input)
    {
        ...
        if (input.Contract == null) throw new UserFriendlyException("必须选择一个合同");

        // 1. 使用 Manager 创建带有自动编号的实体
        var entity = await _manager.CreateAsync(input.Contract.Id);
        ...
        var dto = ObjectMapper.Map<Payment, PaymentDto>(entity);
